...
session_name = "lora_bert_meanpool_clsv2"
...
head_path = "classifier.pt"  # Only for LoRA_BERT_MeanPoolClsV2
...
from models import LoRA_BERT_MeanPoolClsV2

if checkpoint_dir is not None:
    model_load_path = checkpoint_dir
    head_load_path = f"{checkpoint_dir}/{head_path}"
else:
    model_load_path = None
    head_load_path = None

model = LoRA_BERT_MeanPoolClsV2(
    from_pretrained=model_load_path,
    from_pretrained_head=head_load_path,
    device=device
).to(device)
...
class SaveLoRA_BERT_MeanPoolClsV2HeadOnSave(TrainerCallback):
    def on_save(self, args, state, control, **kwargs):
        model = kwargs["model"]
        # Match Trainer’s checkpoint folder name
        ckpt_dir = os.path.join(args.output_dir, f"checkpoint-{state.global_step}")
        os.makedirs(ckpt_dir, exist_ok=True)
        # Save the custom head separately: LoRA_BERT_MeanPoolClsV2 only
        assert hasattr(model, "classifier")
        torch.save(model.classifier.state_dict(), f"{ckpt_dir}/{head_path}")
...
if isinstance(model, LoRA_BERT_MeanPoolClsV2):
    trainer_callbacks.append(SaveLoRA_BERT_MeanPoolClsV2HeadOnSave())
